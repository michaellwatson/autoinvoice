...
$( document ).ready(function() {
  ...
  $(document.body).on('click', '.import' ,function(e){
    ...
    $.ajax({
        type: "POST",
        url: base_url+'Documents/updateImport',
  ...
  $(document.body).on('click', '.btn-pdf' ,function(e){
    var id = $(this).attr('data-id');
    var include = $(this).attr('data-include');
    var $element = $(this);
    ...
    $.ajax({
        type: "POST",
        url: base_url+'Documents/updatePdf',
        data: {'id': id,'include': include},
        dataType:'json',
        success: function(data)
        {      
          if(data.status=='1'){
            if(include==1){
              $element.find('i').removeClass('unselected_search').addClass('selected_search');
              $element.find('i').attr('data-include', 0);
            }else{
              $element.find('i').removeClass('selected_search').addClass('unselected_search');
              $element.find('i').attr('data-include', 1);
            }
          }else{
  ...
  $(document.body).on('click', '.btn-word' ,function(e){
    var id = $(this).attr('data-id');
    var include = $(this).attr('data-include');
    var $element = $(this);
    ...
    $.ajax({
        type: "POST",
        url: base_url+'Documents/updateWord',
        data: {'id': id,'include': include},
        dataType:'json',
        success: function(data)
        {      
  ...
  $(document.body).on('click', '.btn-api' ,function(e){
    var id = $(this).attr('data-id');
    var include = $(this).attr('data-include');
    var $element = $(this);
    ...
    $.ajax({
        type: "POST",
        url: base_url+'Documents/updateApi',
        data: {'id': id,'include': include},
        dataType:'json',
        success: function(data)
        {      
...
$("#saveTableForm").submit(function(e) { 
  //alert('hit');
      ...
      $.ajax({
           type: "POST",
           url: base_url+'Documents/saveTable',
...
$(document.body).on('click', '.delete-row' ,function(e){
  ...
  $.ajax({
    type: "POST",
    url: base_url+'/admin/delete_table',
    data: {'table_id': id},
    dataType:'json',
    success: function(data){
      $('#tab_'+id).remove();
