...
?>
	<?php
	if(isset($ptBlocksMultipleDatasId)){
	}
	else{
	?>
	<div class="alert alert-light" role="alert" style="color: #0c5460; background-color: #d1ecf1; border-color: #bee5eb;">
		To save the repeated section, click the 'Save' button. Your changes will be saved, and the page will automatically reload.
	</div>
	<?php
	}
	?>
	<form name="frmBalconiesRepeat">
	<div style="" id="pos_<?php echo $adId; ?>_repeat_<?php echo $newId; ?>">
<?php
		foreach($fields as $f){
		  ...
		  $data['additionalFormType']   = $additionalFormType;
		  ...
		  if(!is_null($result)){
		    ...
		    <div class="row"  id="id_<?php echo $f['adv_id']?>">
		              ...
		              <?php if($f['adv_post_text']!=''){?>
		              	<div class="col-md-3 col-sm-3 <?php if(is_array($missing_links)){ if(in_array($f['adv_column'], $missing_links)){ echo 'bg-warning'; } }?>">
		              <?php }else{?>
		              	<!--<div class="col-md-<?php if($f['fi_type']!='textarea'){?>6<?php }else{ ?>12<?php } ?> col-sm-<?php if($f['fi_type']!='textarea'){?>6<?php }else{ ?>12<?php } ?>">-->
		                <div class="col-md-6 col-sm-6 <?php if(is_array($missing_links)){ if(in_array($f['adv_column'], $missing_links)){ echo 'bg-warning'; } }?>">
		              <?php } ?>
		                    <?php       
		                      $i = 1;
		                      $html2 = '';
...
?>
	...
	<input type="hidden" name="hdnNewId" value="<?php echo $newId; ?>">
	<input type="hidden" name="hdnRepeaterAdvCategory" value="<?php echo $advCategory; ?>">
	<input type="hidden" name="hdnBalconiesPhotoUpload">
	<input type="hidden" name="hdnBalconiesFileMerger">
	<?php
	if(isset($ptBlocksMultipleDatasId)){
	?>
		<input type="hidden" name="pt_blocks_multiple_datas_id" value="<?php echo $ptBlocksMultipleDatasId; ?>">
	<?php
	}
	?>
	<?php
	if(isset($listingAdId) && !empty($listingAdId)){
	?>
		<input type="hidden" name="hdnListingAdId" value="<?php echo $listingAdId; ?>">
	<?php
	}
	?>
	<?php
	...
	?>
		<input type="hidden" name="hdnTempAddImage" value="">
		<input type="hidden" name="hdnTempAddFileMerger" value="">
	<?php
	}
	?>
	<button type="button" class="btn btn-primary ladda-button clsBtnBalconiesRepeatSubmit" data-style="expand-right" id="idBtnBalconiesRepeatSubmit">Save</button>
	</form>
<?php
}
?>