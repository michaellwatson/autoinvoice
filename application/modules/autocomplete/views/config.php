<i class="fa fa-search au_search" aria-hidden="true" data-toggle="modal" data-target="#<?php echo $d['adv_column'];?>"></i> <i class="fa fa-television au_display" aria-hidden="true" data-toggle="modal" data-target="#<?php echo $d['adv_column'];?>"></i> <i class="fa fa-plus-square au_add_inline" aria-hidden="true" data-toggle="modal" data-target="#<?php echo $d['adv_column'];?>_button"></i>
...
<div class="modal" tabindex="-1" role="dialog" aria-labelledby="<?php echo $d['adv_column'];?>_button" aria-hidden="true" id="<?php echo $d['adv_column'];?>_button"> 
    <div class="modal-dialog"> 
        <div class="modal-content"> 
            <div class="modal-header"> 
                <h5 class="modal-title au_title">
            ...
            <div class="modal-body">
                ...
                <div class="form-check">
                  <input class="form-check-input" type="checkbox" value="" id="display_button">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary <?php echo $d['adv_column'];?>_save2 ladda-button" data-style="expand-right">Save</button>
...
$( document ).ready(function() {
    ...
    $(document).on('click', '.au_add_inline', function(e){
        config = 'add';
    });
    ...
    $(document).on('click', '.<?php echo $d['adv_column'];?>_save2', function(e){
        var id = $(this).attr('data-id');

        var l = Ladda.create(document.querySelector('.<?php echo $d['adv_column'];?>_save2'));
        l.start();

        e.preventDefault();

        let val = $('#display_button').is(':checked');
        $.ajax({
            type: "POST",
            url: base_url+'autocomplete/Autocomplete/save_config',
            data: {'config': config,'data': val, 'field': <?php echo $d['adv_id'];?>}, // serializes the form's elements.
            ...
            success: function(data){
                ...
                console.log(data);
                toastr.success(data.msg);
                $("#<?php echo $d['adv_column'];?>_button").modal('hide');
            } 
        });
    });
});
